...
class B3_Comment extends B3_API {
	...
	public function register_routes($routes) {
		...
		$comment_routes = array(
			'/(posts|pages|media)/(?P<id>\d+)/b3_replies' => array(
			...
			'/b3_comments/(?P<id>\d+)/b3_replies' => array(
	 ...
	 *
	 * - Changes the reply link to use the `/{type}/{id}/b3_replies` endpoint.
	 *
	...
	public function prepare_post($_post, $post, $context) {
		...
		if ('view-revision' !== $context) {
			...
			$replies = preg_replace('/\/comments$/', '/b3_replies', $replies);
			$_post['meta']['links']['b3_replies'] = $replies;
		}
	...
	protected function prepare_comment_meta($comment, $context = 'single') {
		...
		if (0 !== (int) $comment->comment_parent) {
			$meta['links']['in-reply-to'] = json_url(sprintf('/b3_comments/%d', (int) $comment->comment_parent));
		}

		if ('single' !== $context) {
			$meta['links']['self'] = json_url(sprintf('/b3_comments/%d', (int) $comment->comment_ID));
		}
